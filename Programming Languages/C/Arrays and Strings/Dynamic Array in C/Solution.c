 ...
 * Input Format
 *
 * The first line contains an integer 'total_number_of_shelves', the number of
 * shelves in the library.
 *
 * The second line contains an integer 'total_number_of_queries', the number of
 * requests.
 *
 * Each of the following 'total_number_of_queries' lines contains a request in
 * one of the three specified formats.
 *
 * Constraints
 *
 ...
 * Output Format
 *
 * Write the logic for the requests of type 1. The logic for requests of types 2
 * and 3 is provided.
 *
 * Sample Input
 *
...
int main() {
    ...
    // Allocate memory according to the 'total_number_of_shelves' parameter.
    total_number_of_books = (int*) malloc(total_number_of_shelves*sizeof(int));
    total_number_of_pages = (int**) malloc(total_number_of_shelves*sizeof(int*));

    // Initialize values for 'total_number_of_books' and
    ...
    while (total_number_of_queries--) {
        ...
        if (type_of_query == 1) {
            ...
            // Increase the total number of books in shelf 'x' by one.
            total_number_of_books[x]++;

            // Dynamically increase the size of the array of references of the
            // number of pages of each book in shelf 'x'.
            total_number_of_pages[x] = realloc(total_number_of_pages[x],
                                               total_number_of_books[x] *
                                               sizeof(int));

            // Store the book with 'y' number of pages at the end of shelf 'x'.
            // We use 'total_number_of_books - 1' to account for 0-indexing in
            // the array.
