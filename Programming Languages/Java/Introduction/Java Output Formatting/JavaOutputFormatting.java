 ...
 * characters. The second column contains the integer, expressed in exactly 3
 * digits; if the original input has less than three digits, you must pad your
...
public class JavaOutputFormatting {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            tmp = in.nextInt();
            if (0 <= tmp && tmp <= 9) {
                stringArr[i] = stringArr[i] + "00" + tmp;
            } else if (10 <= tmp && tmp <= 99) {
                stringArr[i] = stringArr[i] + "0" + tmp;
            } else {
                stringArr[i] = stringArr[i] + tmp;
            }
        }
        in.close();
        System.out.println("================================");
