 ...
 *
 * For example, -4 and -5 would result in 'java.lang.Exception: n or p should
 * not be negative.'.
 *
 ...
 * Input Format
 *
 * Each line of the input contains two integers, 'n' and 'p'. The locked stub
 * code in the editor reads the input and sends the values to the method as
 * parameters.
 *
 * Constraints
 *
 ...
 * Explanation
 *
 * * In the first two cases, both 'n' and 'p' are positive. So, the power
 *   function returns the answer correctly.
 *
 * * In the third case, both 'n' and 'p' are zero. So, the exception, "n and p
 *   should not be zero.", is printed.
 *
 * * In the last two cases, at least one out of 'n' and 'p' is negative. So, the
 *   exception, "n or p should not be negative.", is printed for these two
 *   cases.
 */
...
class MyCalculator {
    ...
    public long power(int n, int p) throws Exception {
        ...
        if (n < 0 || p < 0) {
            ...
            res = (long) Math.pow((double) n, (double) p);
        }
        return res;
    }
}
...
public class JavaExceptionHandling {
    ...
    public static void main(String[] args) {
        ...
        while (in.hasNextInt()) {
            ...
            try {
                res.add(Long.toString(myCalculator.power(n, p)));
            } catch (Exception e) {
                res.add(e.toString());
            }
        }
