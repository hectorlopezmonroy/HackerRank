 ...
 * integers named 'myArray' that can hold four integer values, you would write
 * the following code:
 *
 ...
 * of 'myArray', we can store integers at indices 0, 1, 2, and 3. Let's say we
 * wanted the last cell to store the number 12; to do this, we write:
 *
 *      myArray[3] = 12;
 *
 * Similarly, we can print the contents of the last cell with the following
 * code:
 *
 *      System.out.println(myArray[3]);
 *
 * The code above prints the value stored at index 3 of 'myArray', which is 12
 * (the value we previously stored there). It's important to note that while
 * Java initializes each cell of an array of integers with a 0, not all
 * languages do this.
 *
 * Task
 * The code in your editor does the following:
 *
 *   1. Reads an integer from stdin and saves it to a variable, 'n', denoting
 ...
 * array. Each line 'i' of the 'n' subsequent lines contains a single integer
 * denoting the value of element 'a_i'.
 *
 * Output Format
 *
 * You are not responsible for printing any output to stdout. Locked code in the
 * editor loops through array 'a' and prints each sequential element on a new
 * line.
 *
 * Sample Input
 *
 ...
 * a = [10, 20, 30, 40, 50]. The locked code prints each array element on a new
