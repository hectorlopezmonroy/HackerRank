 ...
 *
 * Given 2 BitSets, B_1 and B_2, of size 'N' where all bits in both BitSets are
 * initialized to 0, perform a series of 'M' operations. After each operation,
 ...
 *   * SET <set> <index>
 *
 * In the list above, <set> is the integer 1 or 2, where 1 denotes B_1 and 2
 * denotes B_2. <index> is an integer denoting a bit's index in the BitSet
 * corresponding to <set>.
 *
 * For the binary operations AND, OR, and XOR, operands are read from left to
 * right and the BitSet resulting from the operation replaces the contents of
 * the 'first operand'. For example:
 *
 *   AND 2 1
 *
 * B_2 is the left operand, and B_1 is the right operand. This operation should
 * assign the result of B_2 /\ B_1 to B_2.
 *
 * Constraints
 *
 * 1 <= N <= 1000
 ...
 * Output Format
 *
 * After each operation, print the respective number of 'set bits' in BitSet B_1
 * and BitSet B_2 as 2 space-separated integers on a new line.
 *
 * Sample Input
 *
 * 5 4
 ...
 * At each step, we print the respective number of 'set bits' in B_1 and B_2 as
 * a pair of space-separated integers on a new line.
 *
 * M_0 = AND 1 2
