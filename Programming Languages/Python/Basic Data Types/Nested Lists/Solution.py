...
# The '2 * N' subsequent lines describe each student over '2' lines; the first
# line contains a student's name, and the second line contains their grade.
#
# Constraints
#
# 2 <= N <= 5
...
if __name__ == '__main__':
    res = []
    students = []
    lowestGrade = sys.maxsize;
    secondLowest = sys.maxsize;

    # Process input and builds the nested list
    # of students' names and their grades.
    for _ in range(int(input())):
        ...
        if score < lowestGrade:
            secondLowest = lowestGrade
            lowestGrade = score
        elif score < secondLowest and lowestGrade < score:
            secondLowest = score

    # Loops through the nested list and adds the student's name of those
    # students with the second lowest grade to the 'res' list.
    for i in students:
        if i[1] == secondLowest:
            res.append(i[0])

    # Orders the 'res' list of names alphabetically.
    res.sort()
